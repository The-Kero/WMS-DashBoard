...
def compare_pixels():
    ...
    print("\n" + "="*70)
    print("PIXEL WIDTH COMPARISON (Width property)")
    print("="*70)
    ...
    for col in ['A','B','C','D','E','F','G','H']:
        ...
        s_width = sample.range(f'{col}1').api.Width  # points
        s_px = s_width * 96 / 72
        ...
        print(f"{col:<4} {s_cw:<12.2f} {s_px:<12.0f} {g_cw:<12.2f} {g_px:<12.0f} {px_diff:<+10.0f}")
    
    print("\n" + "="*70)
    print("What we need to set:")
    print("="*70)
    for col in ['A','B','C','D','E','F','G','H']:
        s_cw = sample.range(f'{col}1').api.ColumnWidth
        print(f"'{col}': {s_cw},")
