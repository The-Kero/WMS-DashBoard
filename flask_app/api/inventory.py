...
def get_inventory():
    ...
    """
    try:
        logger.info("재고 데이터 조회 시작")
        
        # 오늘 날짜로 파일 경로 설정
        today = datetime.now().strftime("%Y%m%d")
        file_path = f"C:/OSIS_AUTO/inventory_status/inventory_status_{today}.csv"
        ...
        logger.info(f"재고 데이터 조회 성공: {len(data)}건")
        ...
        return jsonify({
            'success': False,
            'error': '파일을 찾을 수 없습니다',
            'message': f'inventory_status_{today}.csv 파일이 없습니다'
        }), 404
        ...
        logger.error(f"재고 API 오류: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'error': '서버 오류',
            'message': str(e)
