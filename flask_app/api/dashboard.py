...
# 재고조사 대상 상품코드 필터 import
try:
    from services.inventory_check_filter import get_target_product_codes
    INVENTORY_FILTER_AVAILABLE = True
except ImportError as e:
    logger.warning(f"재고조사 필터 import 실패: {e}")
    INVENTORY_FILTER_AVAILABLE = False
...
def check_inventory_excel_exists(date_str):
    ...
    combined_file = Path(INVENTORY_EXCEL_OUTPUT_DIR) / f"{file_date} 냉장 재고조사(신선&임박상품).xlsx"
    return combined_file.exists()
...
def get_dashboard():
    ...
    try:
        ...
        # ==========================================
        # 0. 시스템 상태 (5개 모듈 로그 파일 체크)
        # ==========================================
        log_files = {
            '입고': f"{base_path}/Inbound Status/inbound.log",
            '출고': f"{base_path}/Outbound Status/outbound.log",
            '재고': f"{base_path}/inventory_status/inventory.log",
            '삭제': f"{base_path}/Delete Status/delete.log",
            '비정형': f"{base_path}/IrregularOrder Status/irregular.log"
        }
        
        system_status = 'green'  # 기본값
        now = datetime.now()
        ...
        for module_name, log_path in log_files.items():
            try:
                ...
                if diff_seconds > 300:
                    ...
                    module_status = 'orange'
                else:
                    module_status = 'green'
                
                # 더 나쁜 상태로 업데이트
                status_priority = {'red': 0, 'orange': 1, 'green': 2}
                if status_priority.get(module_status, 2) < status_priority.get(system_status, 2):
                    system_status = module_status
                    
            except FileNotFoundError:
                system_status = 'red'  # 파일 없으면 중단 상태
                logger.warning(f"로그 파일 없음: {module_name} ({log_path})")
            except Exception as e:
                logger.warning(f"로그 파일 체크 실패: {module_name} - {str(e)}")
        ...
        # ==========================================
        # 2-0. 재고 데이터 (초록불 판정에 필요하여 앞으로 이동)
        # ==========================================
        ...
        # --- 재고조사 대상 상품코드 추출 (새 로직) ---
        target_codes = set()
        use_filtered_logic = False
        
        if INVENTORY_FILTER_AVAILABLE:
            try:
                target_codes = get_target_product_codes(inventory_data)
                if len(target_codes) > 0:
                    use_filtered_logic = True
                    logger.info(f"재고조사 필터 적용: {len(target_codes)}개 상품코드")
                else:
                    logger.info("재고조사 필터 결과 없음 - 기존 로직 사용")
            except Exception as e:
                logger.warning(f"재고조사 필터 실패: {str(e)} - 기존 로직 사용")
        else:
            logger.info("재고조사 필터 미사용 - 기존 로직 사용")
        
        # --- 출고 상태 계산 ---
        if len(outbound_data) > 0 and '문서상태명' in outbound_data.columns:
            # 새 로직: 재고조사 대상 상품만 필터링
            if use_filtered_logic and '상품' in outbound_data.columns:
                filtered_outbound = outbound_data[outbound_data['상품'].astype(str).isin(target_codes)]
                unwork_count = len(filtered_outbound[filtered_outbound['문서상태명'] == '미작업'])
                unalloc_count = len(filtered_outbound[filtered_outbound['문서상태명'] == '미할당'])
                logger.info(f"출고 필터링: 전체 {len(outbound_data)}건 → 대상 {len(filtered_outbound)}건")
            else:
                # 기존 로직: 전체 출고 데이터
                unwork_count = len(outbound_data[outbound_data['문서상태명'] == '미작업'])
                unalloc_count = len(outbound_data[outbound_data['문서상태명'] == '미할당'])
            
            inventory_check_count = unalloc_count  # 미할당만 체크
            ...
            logger.info(f"출고 신호등 - 미작업: {unwork_count}(참고), 미할당: {unalloc_count}, 상태: {outbound_status}")
        
        # --- 입고 진척률 계산 (미입고 목록 제외) ---
        if len(inbound_data) > 0 and '진척률' in inbound_data.columns:
            # 새 로직: 재고조사 대상 상품만 필터링
            if use_filtered_logic and '상품' in inbound_data.columns:
                working_inbound = inbound_data[inbound_data['상품'].astype(str).isin(target_codes)].copy()
                logger.info(f"입고 필터링: 전체 {len(inbound_data)}건 → 대상 {len(working_inbound)}건")
            else:
                working_inbound = inbound_data.copy()
            ...
            if len(exclude_keys) > 0:
                mask = working_inbound.apply(
                    lambda row: (str(row['입고예정번호']), str(row['상품'])) not in exclude_keys, 
                    axis=1
                )
                filtered_inbound = working_inbound[mask]
            else:
                filtered_inbound = working_inbound
        ...
        elif 1160 <= current_total_minutes < 1260:  # 19:20 ~ 21:00
            target_dests = slot2_dests
            target_hour = 18  # 기준값은 18시 파일 사용
            card3_alert_time_slot = "19:20-21:00"
        ...
        # 유효유통비 30% 이하 필터링 (부진재고 기준)
        risky = inventory_data[inventory_data['유효유통비(%)'] <= 30].copy()
        
        # 부진재고존(L06, L07) 로케이션 제외 → 일반존만 남김
        risky_filtered = risky[
            ~risky['로케이션'].str.startswith('L06', na=False) &
            ~risky['로케이션'].str.startswith('L07', na=False)
        ].copy()
        
        # 가용수량 0 제외
        risky_filtered = risky_filtered[risky_filtered['가용수량'] != 0].copy()
        
        # 긴급/주의 구분 (부진재고 기준: 15% 이하 긴급, 16~30% 주의)
        card5_urgent = len(risky_filtered[risky_filtered['유효유통비(%)'] <= 15])
        card5_warning = len(risky_filtered[risky_filtered['유효유통비(%)'] > 15])
