...
**시작일**: 2025-11-16 (토)  
**예상 완료일**: 2025-11-28 (금)  
**현재 진척률**: 84.2% (415/493 작업 완료)
**최종 업데이트**: 2025-11-27 13:15
**주요 수정**: Day 4 PART 4 완료 - 5분 자동 갱신 + 운영시간 체크 + 자정 새로고침
...
| Day 4 | 2025-11-26 (수) | **API 재설계 + v10 변환 + 30초 갱신** | **95개** | 90 | 94.7% | 🔄 진행중 | - |
| Day 5 | 2025-11-27 (목) | 성능 최적화 + 안정성 | 28개 | 0 | 0% | ⏳ 대기 | - |
| Day 6 | 2025-11-28 (금) | 배포 + TV 검증 | 45개 | 0 | 0% | ⏳ 대기 | - |
| **합계** | **6일** | **Flask TV 시스템** | **493개** | **415** | **84.2%** | 🔄 | **7.3시간** |

---

## ✅ Day 1: Flask 기본 구조 + Collector 모듈 (완료)
...
| PART | 내용 | 체크박스 | 난이도 | 상태 |
|:----:|------|:--------:|:------:|:----:|
| PART 1 | API 응답 구조 재설계 (카드번호 + 키이름) | 25개 | 중 | ✅ 완료 |
| PART 2 | card6 API 신규 개발 (금일재고현황 5섹션) | 30개 | **상** | ✅ 완료 |
| PART 3 | v10.html → Flask 템플릿 변환 | 15개 | 하 | ✅ 완료 |
| PART 4 | 30초 자동 갱신 JavaScript | 20개 | 중 | ✅ 완료 |
| PART 5 | 통합 테스트 | 5개 | 하 | ⏳ 대기 |

---

### 🔴 PART 1: API 응답 구조 재설계 (25개) ✅ 완료
...
- [x] 1.1.1 dashboard.py 백업 생성
- [x] 1.1.2 기존 card2 → card1로 변수명 변경 (입고현황)
- [x] 1.1.3 기존 card3 → card5로 변수명 변경 (피킹유의)
- [x] 1.1.4 기존 card5 → card2로 변수명 변경 (자사출고)
- [x] 1.1.5 기존 card6 → card3로 변수명 변경 (지방출고)
- [x] 1.1.6 JSON 응답에서 card1 키 사용
- [x] 1.1.7 JSON 응답에서 card2 키 사용
- [x] 1.1.8 JSON 응답에서 card3 키 사용
- [x] 1.1.9 JSON 응답에서 card5 키 사용
- [x] 1.1.10 카드 번호 매핑 주석 추가
...
**card1 (입고현황) 키 변경:**
- [x] 1.2.1 totalCount 유지
- [x] 1.2.2 progressRate 유지
- [x] 1.2.3 inboundRiskyCount 유지
- [x] 1.2.4 riskyItems 배열 추가 (기존 API에 없음!)

**card2 (자사출고) 키 변경:**
- [x] 1.2.5 totalAmount → amount 변경
- [x] 1.2.6 comparePercent → compare 변경
- [x] 1.2.7 unpublishedLabel → unprintedLabel 변경
- [x] 1.2.8 irregularUnpublished → irregularUnprinted 변경

**card3 (지방출고) 키 변경:**
- [x] 1.2.9 totalAmount → amount 변경
- [x] 1.2.10 destinations 객체 형태로 변경 (배열 → 객체)
- [x] 1.2.11 unprintedPicking 필드 추가

**card5 (피킹유의) 키 변경:**
- [x] 1.2.12 items → riskyItems 변경
- [x] 1.2.13 riskyItems 내부 키 확인 (location, productCode, productName, expiryDate, stockQty, validityRatio)
- [x] 1.2.14 totalCount, urgentCount, warningCount 유지
- [x] 1.2.15 키 변경 완료 테스트

**완료 시간**: 2025-11-26 21:45 ✅

---

### 🔴 PART 2: card6 API 신규 개발 (30개) ✅ 완료 + 로직검증완료

**⚠️ 로직 검증 완료 (2025-11-27)**
- 수정1: 비정형 오더 예외처리 추가
- 수정2: 적치율 계산 → CSV 파일 기준 (37.6% → 71.8%)
- 수정3: FIFO 위반 계산 → CSV 파일 기준 (7건 → 1건)
- 핵심: 상단 = "냉장 파레트 적재 로케이션.csv" 524개 기준
...
- [x] 2.1.1 card6 계산용 변수 초기화 블록 추가
- [x] 2.1.2 inventory_status 파일 경로 설정
- [x] 2.1.3 outbound_all 파일 경로 설정
- [x] 2.1.4 delete_status 파일 경로 설정
- [x] 2.1.5 inbound_status 파일 경로 설정 (이미 있음)
- [x] 2.1.6 각 CSV 로드 예외 처리
- [x] 2.1.7 card6 계산 시작 주석 블록
- [x] 2.1.8 디버그 로그 추가

**완료 시간**: 2025-11-26 22:12 ✅

---

#### 2.2 섹션1: 총재고 + 적치율 계산 (5개)

- [x] 2.2.1 총재고 계산 (가용수량 × 단가)
- [x] 2.2.2 적치율 계산 (L로 시작하는 로케이션)
- [x] 2.2.3 524 상수 정의 (전체 상단 로케이션 수)
- [x] 2.2.4 inventory 객체 생성 { total_value }
- [x] 2.2.5 storageRate 객체 생성 { rate }

**완료 시간**: 2025-11-26 22:15 ✅ (기존 구현 확인)

---

#### 2.3 섹션2: 선입선출 위반 계산 (5개)

- [x] 2.3.1 상단(L) 로케이션 필터링
- [x] 2.3.2 하단(K) 로케이션 필터링
- [x] 2.3.3 상품별 소비기한 비교 로직
- [x] 2.3.4 위반 항목 리스트 생성
- [x] 2.3.5 fifo 배열 생성 [{ location, productCode, productName, upperExpiry, lowerExpiry }]

**완료 시간**: 2025-11-26 22:17 ✅

---

#### 2.4 섹션3: 미출/부분출고 계산 (5개)

- [x] 2.4.1 18시 체크 로직 (현재시간 >= 18)
- [x] 2.4.2 백업 파일 경로 설정
- [x] 2.4.3 현재 vs 백업 비교 로직
- [x] 2.4.4 부분출고/미출 분류
- [x] 2.4.5 partial 배열 생성 [{ delivery_group, delivery_name, product_name, unit, original_qty, order_qty }]

**완료 시간**: 2025-11-26 22:20 ✅
...
- [x] 2.5.1 안산 삭제 필터링
- [x] 2.5.2 음성 삭제 필터링
- [x] 2.5.3 deleteAnsan 배열 생성 [{ name, unit, count }]
- [x] 2.5.4 deleteEumseong 배열 생성 [{ name, unit, count }]

**완료 시간**: 2025-11-26 22:26 ✅

---

#### 2.6 섹션5: 미할당 입고현황 계산 (3개)

- [x] 2.6.1 미할당 조건 필터링 (로케이션 비어있음)
- [x] 2.6.2 센터 정보 추출
- [x] 2.6.3 unallocated 배열 생성 [{ name, unit, qty, center }]

**완료 시간**: 2025-11-26 22:26 ✅

**PART 2 전체 완료**: 2025-11-27 10:30 (로직 검증 포함) ✅

---

### 🟡 PART 3: v10.html → Flask 템플릿 변환 (15개) ✅ 완료

#### 3.1 파일 복사 및 구조 변경 (10개)

- [x] 3.1.1 v10.html 파일 백업
- [x] 3.1.2 v10.html → flask_app/templates/dashboard.html 복사
- [x] 3.1.3 templates 폴더 확인
- [x] 3.1.4 dashboard.html 파일 확인
- [x] 3.1.5 app.py에 render_template import 확인
- [x] 3.1.6 루트 라우트 수정 (render_template 사용)
- [x] 3.1.7 Flask 서버 실행
- [x] 3.1.8 localhost:5000 접속 테스트
- [x] 3.1.9 v10 레이아웃 렌더링 확인
- [x] 3.1.10 기본 스타일 적용 확인

**완료 시간**: 2025-11-27 12:33 ✅

---

#### 3.2 정적 파일 설정 (5개)

- [x] 3.2.1 static/css 폴더 확인
- [x] 3.2.2 static/js 폴더 확인
- [x] 3.2.3 로고 이미지 복사 (있다면)
- [x] 3.2.4 Flask static 경로 테스트
- [x] 3.2.5 브라우저에서 정적 파일 로드 확인

**완료 시간**: 2025-11-27 12:35 ✅

---

### 🟡 PART 4: 5분 자동 갱신 JavaScript (20개) ✅ 완료

**⚠️ 설계 변경 (2025-11-27)**
- 30초 → **5분 간격**으로 변경 (백엔드 수집 주기에 맞춤)
- **2분, 7분, 12분...** 에 갱신 (백엔드 완료 후 안전 마진)
- **운영 시간 체크**: 09:00~24:00, 토요일 제외
- **자정 자동 새로고침**: 메모리 관리

#### 4.1 API 호출 함수 추가 (10개)

- [x] 4.1.1 dashboard.html 하단 script 영역 찾기
- [x] 4.1.2 dashboardData 하드코딩 제거 계획
- [x] 4.1.3 async function fetchDashboardData() 함수 추가
- [x] 4.1.4 fetch('/api/dashboard') 호출 로직
- [x] 4.1.5 response.json() 파싱
- [x] 4.1.6 에러 처리 try-catch
- [x] 4.1.7 네트워크 에러 시 이전 데이터 유지 (lastData)
- [x] 4.1.8 로딩 상태 표시 (생략 - 불필요)
- [x] 4.1.9 데이터 수신 후 updateCard 함수들 호출
- [x] 4.1.10 console.log 디버그 메시지

**완료 시간**: 2025-11-27 13:10 ✅

---

#### 4.2 5분 주기 갱신 + 운영시간 체크 (10개)

- [x] 4.2.1 REFRESH_INTERVAL = 300000 (5분) 상수 정의
- [x] 4.2.2 let refreshTimer = null 변수
- [x] 4.2.3 function startAutoRefresh() 정의
- [x] 4.2.4 setInterval(fetchDashboardData, REFRESH_INTERVAL)
- [x] 4.2.5 function stopAutoRefresh() 정의
- [x] 4.2.6 clearInterval(refreshTimer)
- [x] 4.2.7 DOMContentLoaded 이벤트에서 init() 호출
- [x] 4.2.8 운영 시간 체크 후 자동 갱신 시작
- [x] 4.2.9 5분 대기 후 갱신 확인
- [x] 4.2.10 console에서 갱신 로그 확인

**완료 시간**: 2025-11-27 13:12 ✅

**추가 구현 (체크박스 외):**
- isOperatingTime() - 운영 시간 체크 (09~24시, 토요일 제외)
- showOffTimeMessage() / hideOffTimeMessage() - 업무 시작 전/휴무일 표시
- scheduleMidnightReload() - 자정 새로고침 예약 (메모리 관리)
- getNextRefreshTime() - 다음 X분 2초 계산
- scheduleOperatingStart() - 09:02까지 대기 후 시작

**PART 4 전체 완료**: 2025-11-27 13:15 ✅
...
> ⚠️ **삭제된 Day**: 기존 Day 5 (JS 50개), Day 6 (CSS 37개)는 v10.html에 이미 완성되어 삭제됨

---

## ✅ Day 5: 성능 + 안정성 (2025-11-27) ⭐ Day 7에서 변경
...
### 5.1 메모리 최적화 (30분)
- [ ] 5.1.1 dashboard.js 메모리 누수 확인
- [ ] 5.1.2 이벤트 리스너 정리 로직 추가
- [ ] 5.1.3 타이머 정리 확인
- [ ] 5.1.4 파일 저장
...
### 5.6 Git 커밋 (30분)
- [ ] 5.6.1 `git add .` 실행
- [ ] 5.6.2 `git commit -m "Phase 2 Day 5: 성능+안정성 완성"` 실행
- [ ] 5.6.3 커밋 확인
...
**Day 5 최종 완료율**: ___% (___/28개 완료)

---

## ✅ Day 6: 배포 + 검증 (2025-11-28) ⭐ Day 8에서 변경
...
### 6.1 환경 설정 (90분)
- [ ] 6.1.1 .env.production 파일 생성
- [ ] 6.1.2 FLASK_ENV=production 설정
- [ ] 6.1.3 FLASK_DEBUG=False 설정
- [ ] 6.1.4 LOG_LEVEL=WARNING 설정
- [ ] 6.1.5 파일 저장
...
### 6.2 NSSM 설치 및 설정 (75분)
- [ ] 6.2.1 NSSM 다운로드
- [ ] 6.2.2 C:\Program Files\NSSM 폴더 생성
- [ ] 6.2.3 NSSM 압축 해제
- [ ] 6.2.4 관리자 PowerShell 열기
- [ ] 6.2.5 `nssm install WMS-Flask-Dashboard` 실행
- [ ] 6.2.6 Application Path 설정
- [ ] 6.2.7 Startup directory 설정
- [ ] 6.2.8 Arguments 설정
- [ ] 6.2.9 서비스 생성 완료
- [ ] 6.2.10 `nssm start WMS-Flask-Dashboard` 실행
- [ ] 6.2.11 서비스 시작 확인
...
### 6.3 100인치 TV 테스트 (90분)
- [ ] 6.3.1 TV IP 확인 (10.60.27.130)
- [ ] 6.3.2 네트워크 연결 확인
- [ ] 6.3.3 Chrome 전체화면 바로가기 생성
- [ ] 6.3.4 --kiosk 모드 설정
- [ ] 6.3.5 TV에서 브라우저 실행
- [ ] 6.3.6 localhost:5000 접속
- [ ] 6.3.7 전체 화면 확인
- [ ] 6.3.8 가독성 테스트 (5m 거리)
- [ ] 6.3.9 30초 자동 갱신 확인
- [ ] 6.3.10 6개 카드 데이터 확인
...
#### 8.4 전체 기능 검증 (90분)
- [ ] 6.4.1 /api/health 동작 확인
- [ ] 6.4.2 /api/dashboard 동작 확인
- [ ] 6.4.3 5개 개별 API 동작 확인
- [ ] 6.4.4 30초 갱신 정확도 확인
- [ ] 6.4.5 에러 처리 확인
- [ ] 6.4.6 로그 기록 확인
- [ ] 6.4.7 성능 측정 (페이지 로딩 < 2초)
- [ ] 6.4.8 메모리 사용량 확인 (< 500MB)

**완료 시간**: 2025-11-20 15:15 ✅

---

### 6.5 Phase 2 완료 보고서 (30분)
- [ ] 6.5.1 완료 항목 체크리스트 작성
- [ ] 6.5.2 성능 지표 정리
- [ ] 6.5.3 트러블슈팅 이력 정리
- [ ] 6.5.4 다음 단계 (Phase 3) 준비사항
- [ ] 6.5.5 파일 저장
...
### 6.6 Git 최종 커밋 및 태그 (30분)
- [ ] 6.6.1 `git add .` 실행
- [ ] 6.6.2 `git commit -m "Phase 2 완료: Flask TV 시스템 배포"` 실행
- [ ] 6.6.3 `git tag -a v2.0 -m "Phase 2 완료"` 실행
- [ ] 6.6.4 `git push origin main` 실행
- [ ] 6.6.5 `git push origin v2.0` 실행
- [ ] 6.6.6 GitHub에서 태그 확인
